...
void VideoForm::toggleXaiDebugForm()
{
    if (!m_xaiDebugForm) {
        m_xaiDebugForm = new XaiDebugForm(this, ToolForm::AP_OUTSIDE_LEFT);
        m_xaiDebugForm->setSerial(m_serial);
    }
    
    bool show = !m_xaiDebugForm->isVisible();
    m_xaiDebugForm->move(pos().x() - m_xaiDebugForm->geometry().width(), pos().y() + 30);
    m_xaiDebugForm->setVisible(show);
}

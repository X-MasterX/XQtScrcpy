...
void ToolForm::initStyle()
{
    ...
    IconHelper::Instance()->SetIcon(ui->mcpDebugBtn, QChar(0xf188), 15);
}
...
void ToolForm::on_mcpDebugBtn_clicked()
{
    VideoForm *videoForm = qobject_cast<VideoForm*>(parentWidget());
    if (videoForm) {
        videoForm->toggleXaiDebugForm();
    }
}